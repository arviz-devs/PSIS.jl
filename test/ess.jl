...
@testset "effective sample size" begin
    ...
    @test ess isa Vector
    @test length(ess) == 3
    @test ess[1] ≈ ess_is(result.weights, reff)[1]
    @test ismissing(ess[2])
    @test ismissing(ess[3])
    ess = ess_is(result; bad_shape_missing=false)
    @test ess[1:2] ≈ ess_is(result.weights, reff)[1:2]
    @test ismissing(ess[3])
end
