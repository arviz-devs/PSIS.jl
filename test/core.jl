...
@testset "PSISResult" begin
    @testset "vector log-weights" begin
        ...
        diag = PSIS.ParetoDiagnostics(0.5, 0.7, 100, 1)
        result = PSISResult(log_weights, reff, false, diag)
        @test result isa PSISResult{Float64}
        @test result.log_weights == log_weights
        @test result.reff == reff
        @test !result.normalized
        @test result.diagnostics == diag
        ...
        @testset "show" begin
            @test sprint(show, "text/plain", result) == """
                ...
                Pareto shape (k) diagnostic values:
                                    Count       Min. ESS
                 (-Inf, 0.6]  good  1 (100.0%)  $(floor(Int, ess))"""
        end
    end
    ...
    @testset "array log-weights" begin
        ...
        diag = ParetoDiagnostics(pareto_shape, 0.7, nothing, nothing)
        result = PSISResult(log_weights, reff, true, diag)
        @test result isa PSISResult{Float64}
        @test result.log_weights == log_weights
        @test result.reff == reff
        @test result.diagnostics == diag
