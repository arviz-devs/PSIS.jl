...
@testset "effective sample size" begin
    ...
    @test ess_is(result) ≈ ess_is(result.weights; r_eff=1.5)

    result = PSISResult(logw, 1.5, 20, PSIS.GeneralizedPareto(0.0, 1.0, 0.71), false)
    @test isnan(ess_is(result))
    @test ess_is(result; bad_shape_nan=false) ≈ ess_is(result.weights; r_eff=1.5)
    ...
    r_eff = [1.5, 0.8, 1.0]
    result = PSISResult(logw, r_eff, [20, 20, 20], tail_dists, false)
    ess = ess_is(result)
    @test ess isa Vector
    @test length(ess) == 3
    @test ess[1] ≈ ess_is(result.weights; r_eff)[1]
    @test isnan(ess[2])
    @test isnan(ess[3])
    ess = ess_is(result; bad_shape_nan=false)
    @test ess ≈ ess_is(result.weights; r_eff)[1:3]
end
